...
class UserLogin
{
	...
	public function check_userlogin () {
		...
		if ( empty( $user_id ) ){
			$this->logged_in = false;
			$this->login_error =  "<script type='text/javascript'>Materialize.toast('Usuário não existe', 4000, 'toast_alert');</script>";
		...
		if ( $this->phpass->CheckPassword( $user_password, $fetch['user_password'] ) ) {
			...
			$this->login_error = "<script type='text/javascript'>Materialize.toast('Senha não encontrada!', 4000, 'toast_alert');</script>";
	...
	public function check_user($user_db_id){
		$query = $this->db->query(
			'SELECT * FROM Users WHERE user_id = ' . $user_db_id
		);
